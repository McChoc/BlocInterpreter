        ...
        if (tokens.Count > 0)
        {
            foreach (var part in tokens.Split(x => x is SymbolToken(Symbol.COMMA)))
            {
                var argument = part switch
                {
                    ...
                    [INamedIdentifierToken token, SymbolToken(Symbol.EQUAL), _, ..] => new InvocationOperator.Argument(token.GetIdentifier(), ExpressionParser.Parse(part.GetRange(2..)), ArgumentType.Keyword),
