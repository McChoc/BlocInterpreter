...
internal static partial class ExpressionParser
{
    private static IExpression ParseFunctions(List<Token> tokens, int precedence)
    {
        for (var i = 0; i < tokens.Count; i++)
        {
            ...
            else if (tokens[i] is (TokenType.Symbol, Symbol.LAMBDA) @operator)
            {
                foundFunction = true;
            ...
            if (foundFunction)
            {
                ...
                {
                    foreach (var part in paramTokens.Split(x => x is (TokenType.Symbol, Symbol.COMMA)))
                    {
                        if (part.Count == 0)
                        {
                ...
                while (j >= 0)
                {
                    if (tokens[j] is (TokenType.Word, Keyword.ASYNC))
                    {
                    ...
                    else if (tokens[j] is (TokenType.Word, Keyword.GENERATOR))
                    {
                        if (type == FunctionType.Asynchronous)
                            throw new SyntaxError(tokens[j].Start, tokens[j].End, "a function cannot be both async and a generator");
                    ...
                    else if (tokens[j] is (TokenType.Keyword, Keyword.VAL))
                    {
                    ...
                    else if (tokens[j] is (TokenType.Keyword, Keyword.REF))
                    {
                        if (mode == CaptureMode.Value)
                            throw new SyntaxError(tokens[j].Start, tokens[j].End, "multiple capture modifier");
