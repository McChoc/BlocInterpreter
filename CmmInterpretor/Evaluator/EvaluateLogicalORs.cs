...
namespace CmmInterpretor
{
    ...
    {
        private static IResult EvaluateLogicalORs(List<Token> expr, Call call, int precedence)
        {
            for (int i = expr.Count - 1; i >= 0; i--)
            {
                if (expr[i].type == TokenType.Operator && expr[i].Text == "|")
                {
                    ...
                    var a = EvaluateLogicalORs(expr.GetRange(..i), call, precedence);
                    ...
                    return Operator.Or(aa.Value(), bb.Value());
                }
            }
